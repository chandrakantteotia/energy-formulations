...
export default function RnDSection() {
  return (
    <section id="rnd" className="py-20 lg:py-28 bg-white text-brand-text relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-slate-200 mb-16 bg-brand-navy-950">
          
          {/* Main Laboratory Image */}
          <div className="relative h-[400px] sm:h-[480px] lg:h-[520px] w-full">
            <img
          ...
          {/* Floating Development Journey Overlay (Bottom of Image) */}
          <div className="absolute bottom-6 left-6 right-6 lg:left-10 lg:right-10 glass-dark rounded-2xl p-6 sm:p-8 border border-white/20 shadow-2xl">
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/15">
              <div className="flex items-center gap-2">
                <Atom className="w-4 h-4 text-brand-green-400" />
                <span className="text-xs font-bold uppercase tracking-widest text-brand-green-300">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              {RD_JOURNEY_STEPS.map((step, idx) => (
                <div
                  key={step.step}
                  className="bg-brand-navy-900/80 rounded-xl p-3.5 border border-white/10 flex flex-col justify-between"
                >
                  <span className="text-xs font-mono font-bold text-brand-green-400 mb-1">
                    {step.step}
                  </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {RD_CAPABILITY_AREAS.map((area, idx) => (
            <div
              key={idx}
              className="p-6 rounded-2xl bg-brand-light border border-slate-200/80 hover:border-brand-blue-700/40 hover:bg-white hover:shadow-card-hover transition-all duration-300 flex flex-col justify-between group"
            >
              <div>
          ...
          {/* R&D Inquiry Card */}
          <div className="p-6 rounded-2xl bg-brand-navy-900 text-white border border-brand-navy-800 flex flex-col justify-between">
            <div>
            ...
            <button
              ...
              className="mt-6 w-full py-3 px-4 rounded-xl text-xs font-bold uppercase tracking-wider text-brand-navy-950 bg-brand-green-500 hover:bg-brand-green-400 transition-colors flex items-center justify-center gap-2 shadow-glow-green"
            >
